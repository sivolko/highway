...
export default function CMS() {
  ...
  const publishDraft = async (pr) => {
    ...
    try {
      ...
      setStatus({ type: "success", msg: "Draft published! Deploying...", prUrl: pr.html_url });
      fetchAll();
    } catch (e) { setStatus({ type: "error", msg: e.message }); }
  ...
  const publish = async (draft = false) => {
    ...
    try {
      ...
      if (!editPost?.isDraft) {
        ...
        setAutoSaved(false);
        setStatus({ type: "success", msg: draft ? `Draft PR #${prNumber} created!` : "Published! Deploying...", prUrl: `https://github.com/${owner}/${repo}/pull/${prNumber}`, steps: [] });
      } else {
        if (!draft) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f8f9fa" }}>
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: 40, width: 460, boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
        ...
        {testResult && <div style={{ padding: "10px 14px", borderRadius: 8, marginBottom: 14, fontSize: 13, background: testResult.ok ? "#fff7ed" : "#fef2f2", color: testResult.ok ? "#c2410c" : "#dc2626" }}>{testResult.msg}</div>}
  ...
  return (
    <div style={{ height: "100%", overflow: "auto", padding: 32 }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        {status && <div style={{ padding: "10px 14px", borderRadius: 8, marginBottom: 16, fontSize: 13, background: status.type === "error" ? "#fef2f2" : status.type === "success" ? "#fff7ed" : "#eff6ff", color: status.type === "error" ? "#dc2626" : status.type === "success" ? "#c2410c" : "#1d4ed8" }}>{status.msg}</div>}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "10px 20px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        ...
        {editPost?.isDraft && <span style={{ fontSize: 11, background: "#fff7ed", color: "#c2410c", padding: "3px 8px", borderRadius: 6, fontWeight: 500 }}>Draft</span>}
        <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
          {publishing
            ...
            : <>
                ...
                <button className="btn btn-orange" onClick={() => onPublish(false)} disabled={publishing}>Publish</button>
              </>
          }
        </div>
      </div>
      ...
      <div style={{ display: "grid", gap: 16 }}>
        ...
        {fields.image && (
          <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden" }}>
            <div className="field-label" style={{ padding: "8px 12px", background: "#f8f9fa" }}>Cover Preview</div>
